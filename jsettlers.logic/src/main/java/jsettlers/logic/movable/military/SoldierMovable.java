...
public abstract class SoldierMovable extends AttackableHumanMovable implements ISoldierMovable {
            ...
            resetAfter(mov -> {
                ...
                sequence(
                    ...
                    action(mov -> {
                      ...
                      if (mov.isBowman())
                        mov.inTowerAttackPosition = mov.building.getTowerBowmanSearchPosition(place);
                      mov.isInTower = true;
                    })
                )
            )
                            ...
                            resetAfter(mov -> {
                                ...
                                sequence(
                                    ...
                                    goToPos(mov -> mov.enemy.getPosition(), mov -> {
                                      ...
                                      if (mov.startPoint.getOnGridDistTo(mov.position) > 2)
                                        return false;
                                      return true;
                                    })
                                )
  ...
  private static Node<SoldierMovable> attackEnemy() {
    return sequence(
        ...
        action(mov -> {
          mov.setDirection(EDirection.getApproxDirection(mov.position, mov.enemy.getPosition()));
        }),
  ...
  private void notifyTowerThatRequestFailed() {
    if (building.getPlayer() != player)
      return; // only notify, if the tower still belongs to this player
  ...
  protected float getCombatStrength() {
    ...
    float strengthMod = 1;
    if (alliedGround && hasEffect(EEffectType.DEFEATISM))
      strengthMod *= EEffectType.DEFEATISM_DAMAGE_FACTOR;
    if (!alliedGround && hasEffect(EEffectType.INCREASED_MORALE))
      strengthMod *= EEffectType.INCREASED_MORALE_DAMAGE_FACTOR;
    if (hasEffect(EEffectType.MOTIVATE_SWORDSMAN))
      strengthMod *= EEffectType.MOTIVATE_SWORDSMAN_DAMAGE_FACTOR;
